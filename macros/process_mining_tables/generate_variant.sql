...
Cases_with_variant_ID as (
    select
        ...
        {% if target.type == 'snowflake' -%}
            listagg(Event_log.{{ activity }}, '->') within group (order by Event_log.{{ event_order }})
        {% elif target.type == 'databricks' -%}
            collect_list(Event_log.{{ activity }})
        {% elif target.type == 'sqlserver' -%}
            string_agg(convert(nvarchar(max), Event_log.{{ activity }}), '->') within group (order by Event_log.{{ event_order }})
        {% endif -%}
        as Variant_ID
    from Event_log
