...
{% if ns.execute_test %}
    ...
    {% if test_record_count > 0 %}
        ...
        {% set json_object =
            {"Key": "TestOneColumnNotNull", "Details": {"model_name": "" ~ model.name ~ "", "log_text": "" ~ log_text ~ ""}, "Category": "UserError", "Message": "The table '" ~ model.name ~ "' contains records that have values in multiple fields for " ~ log_text ~ ". Make sure that only one field has a value and the others are NULL in each record."}
        %}
